...
mod tests {
    ...
    #[async_std::test]
    async fn export_import_works_for_empty_wallet() {
    ...
    #[async_std::test]
    async fn export_import_works_for_2_items() {
...

        {
            ...
            let wallet = _wallet("export_import_works_for_2_items2").await;
            _assert_is_empty(&wallet).await;
            ...
            _assert_has_2_records(&wallet).await;
        }
        _cleanup("export_import_works_for_2_items1");
        _cleanup("export_import_works_for_2_items2");
    }
    ...
    #[async_std::test]
    async fn export_import_works_for_2_items_and_interactive_method() {
...

        {
            ...
            let wallet = _wallet("export_import_works_for_2_items_and_interactive_method2").await;
            _assert_is_empty(&wallet).await;
    ...
    #[async_std::test]
    async fn export_import_works_for_multiple_items() {
...

        {
            ...
            let wallet = _wallet("export_import_works_for_multiple_items2").await;
            _assert_is_empty(&wallet).await;
            ...
            _assert_has_300_records(&wallet).await;
        }
    ...
    #[async_std::test]
    async fn import_works_for_empty() {
        _cleanup("import_works_for_empty");

        let res = import(
            &_wallet("import_works_for_empty").await,
            &mut "".as_bytes(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_cut_header_length() {
        _cleanup("import_works_for_cut_header_length");

        let res = import(
            &_wallet("import_works_for_cut_header_length").await,
            &mut "\x00".as_bytes(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_cut_header_body() {
        _cleanup("import_works_for_cut_header_body");

        let res = import(
            &_wallet("import_works_for_cut_header_body").await,
            &mut "\x00\x20small".as_bytes(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_invalid_header_body() {
        ...
        let output = {
            ...
            let mut output: Vec<u8> = Vec::new();
            output
                .write_u32::<LittleEndian>(invalid_header.len() as u32)
                .unwrap();
        ...
        let res = import(
            &_wallet("import_works_for_invalid_header_body").await,
            &mut output.as_slice(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_invalid_header_hash() {
        ...
        let res = import(
            &mut _wallet("import_works_for_invalid_header_hash2").await,
            &mut output.as_slice(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn export_import_works_for_changed_record() {
        ...
        let res = import(
            &mut _wallet("export_import_works_for_changed_record2").await,
            &mut output.as_slice(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_data_cut() {
        ...
        let res = import(
            &mut _wallet("import_works_for_data_cut2").await,
            &mut output.as_slice(),
            _passphrase(),
        )
        .await;
    ...
    #[async_std::test]
    async fn import_works_for_data_extended() {
        ...
        let res = import(
            &mut _wallet("import_works_for_data_extended2").await,
            &mut output.as_slice(),
            _passphrase(),
        )
        .await;
    ...
    async fn _wallet(id: &str) -> Wallet {
        ...
        storage_type
            .create_storage(id, None, None, &metadata)
            .await
            .unwrap();
    ...
    async fn _add_2_records(wallet: Wallet) -> Wallet {
        wallet
            .add(&_type1(), &_id1(), &_value1(), &_tags1())
            .await
            .unwrap();
        wallet
            .add(&_type2(), &_id2(), &_value2(), &_tags2())
            .await
            .unwrap();
        wallet
    }
    ...
    async fn _add_300_records(wallet: Wallet) -> Wallet {
        for i in 0..300 {
            wallet
                .add(&_type(i % 3), &_id(i), &_value(i), &_tags(i))
                .await
                .unwrap();
        }
    ...
    fn _tags(suffix: usize) -> HashMap<String, String> {
        ...
        tags.insert(
            format!("tag_id_{}_1", suffix),
            format!("tag_value_{}_1", suffix),
        );
        tags.insert(
            format!("tag_id_{}_2", suffix),
            format!("tag_value_{}_2", suffix),
        );
        tags.insert(
            format!("~tag_id_{}_3", suffix),
