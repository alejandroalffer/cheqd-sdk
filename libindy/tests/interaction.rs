...
impl Pool {
    pub fn new(pool_name: &str) -> Pool {
        Pool {
            pool_handle: pool::create_and_open_pool_ledger(pool_name).unwrap(),
        }
    }
    ...
    pub fn submit_nym(
        ...
        issuer_wallet_handle: WalletHandle,
        prover_did: &str,
        prover_verkey: Option<&str>,
    ) {
        let nym_request =
            ledger::build_nym_request(issuer_did, prover_did, prover_verkey, None, None).unwrap();
    ...
    pub fn get_schema(&self, did: Option<&str>, schema_id: &str) -> (String, String) {
        ...
        let get_schema_response =
            ledger::submit_request(self.pool_handle, &get_schema_request).unwrap();

        ledger::parse_get_schema_response(&get_schema_response).unwrap()
    }

    pub fn submit_cred_def(
        &self,
        issuer_did: &str,
        issuer_wallet_handle: WalletHandle,
        cred_def_json: &str,
    ...
    pub fn get_cred_def(&self, did: Option<&str>, cred_def_id: &str) -> (String, String) /* (cred_def_id, cred_def_json) */
    {
        ...
        let get_cred_def_response =
            ledger::submit_request(self.pool_handle, &get_cred_def_request).unwrap();

        ledger::parse_get_cred_def_response(&get_cred_def_response).unwrap()
    }

    pub fn submit_revoc_reg_def(
        &self,
        issuer_did: &str,
        issuer_wallet_handle: WalletHandle,
        ...
        let rev_reg_def_request =
            ledger::build_revoc_reg_def_request(issuer_did, rev_reg_def_json).unwrap();
    ...
    pub fn get_revoc_reg_def(&self, did: Option<&str>, revoc_reg_def_id: &str) -> (String, String) /* revoc_reg_def_id, revo_reg_def_json */
    {
        let get_rev_reg_def_request =
            ledger::build_get_revoc_reg_def_request(did, &revoc_reg_def_id).unwrap();

        let get_rev_reg_def_response =
            ledger::submit_request(self.pool_handle, &get_rev_reg_def_request).unwrap();

        ledger::parse_get_revoc_reg_def_response(&get_rev_reg_def_response).unwrap()
    }

    pub fn submit_revoc_reg_entry(
        &self,
        issuer_did: &str,
        issuer_wallet_handle: WalletHandle,
        rev_reg_id: &str,
        ...
        let rev_reg_entry_request = ledger::build_revoc_reg_entry_request(
            issuer_did,
            rev_reg_id,
            REVOC_REG_TYPE,
            rev_reg_entry_json,
    ...
    pub fn get_revoc_reg_delta(
        &self,
        did: Option<&str>,
        revoc_reg_def_id: &str,
        from: Option<u64>,
        to: u64,
    ) -> (String, String, u64) /* rev_reg_id, revoc_reg_delta_json, timestamp */ {
        let get_rev_reg_delta_request =
            ledger::build_get_revoc_reg_delta_request(did, revoc_reg_def_id, from, to).unwrap();

        let get_rev_reg_delta_response =
            ledger::submit_request(self.pool_handle, &get_rev_reg_delta_request).unwrap();
...
impl Issuer {
    pub fn new(pool: &Pool) -> Issuer {
        let (wallet_handle, wallet_config) = wallet::create_and_open_default_wallet(
            format!("wallet_for_pool_{}", pool.pool_handle).borrow(),
        )
        .unwrap();
        ...
        Issuer {
            ...
            // Issuer create DID
            issuer_did: did::create_store_and_publish_my_did_from_trustee(
                wallet_handle,
                pool.pool_handle,
            )
            .unwrap()
    ...
    pub fn create_initial_ledger_state(&mut self, pool: &Pool, revoc_registry_config: &str) {
        ...
        self.issuance_type = String::from(
            revoc_reg_config_value
                .as_object()
                .unwrap()
                .get("issuance_type")
                .unwrap()
        ...
        let _schema_response =
            pool.submit_schema(&self.issuer_did, self.issuer_wallet_handle, &schema_json);
        ...
        let tails_writer_handle =
            blob_storage::open_writer("default", &self.tails_writer_config).unwrap();
    ...
    pub fn make_credential_offer(&self) -> String {
        let cred_offer_json =
            anoncreds::issuer_create_credential_offer(self.issuer_wallet_handle, &self.cred_def_id)
                .unwrap();
    ...
    pub fn issue_credential(
        ...
        let blob_storage_reader_handle =
            blob_storage::open_reader(TYPE, &self.tails_writer_config).unwrap();
        ...
        let (cred_json, cred_rev_id, revoc_reg_delta_json) = anoncreds::issuer_create_credential(
            self.issuer_wallet_handle,
            &cred_offer_json,
            &cred_req_json,
            cred_values_json,
        ...
        if &self.issuance_type == "ISSUANCE_ON_DEMAND" {
            pool.submit_revoc_reg_entry(
                ...
                self.issuer_wallet_handle,
                &self.rev_reg_id,
                &revoc_reg_delta_json.clone().unwrap(),
            );
        }
    ...
    pub fn revoke_credential(&self, pool: &Pool, cred_rev_id: &str) -> String {
        ...
        let rev_reg_delta_json = anoncreds::issuer_revoke_credential(
            self.issuer_wallet_handle,
...
impl Prover {
    pub fn new(master_secret_id: Option<&str>) -> Prover {
        ...
        let (prover_wallet_handle, prover_wallet_config) =
            wallet::create_and_open_default_wallet("interactions_prover").unwrap();
    ...
    pub fn make_credential_request(&mut self, pool: &Pool, cred_offer_json: &str) -> String {
        ...
        let (cred_def_id, cred_def_json) =
            pool.get_cred_def(Some(&self.did), &cred_offer.cred_def_id.0);
    ...
    pub fn store_credentials(&self, pool: &Pool, cred_json: &str, cred_id: &str) {
        ...
        // Prover gets CredentialDefinition from Ledger
        let (_, cred_def_json) =
            pool.get_cred_def(Some(&self.did), &self.cred_def_id.clone().unwrap());

        // Prover gets RevocationRegistryDefinition
        let (_, revoc_reg_def_json) =
            pool.get_revoc_reg_def(None, &credential.rev_reg_id.unwrap().0);
    ...
    pub fn make_proof(
        ...
        let search_handle = anoncreds::prover_search_credentials_for_proof_req(
            self.wallet_handle,
            &proof_request,
            None,
        )
        ...
        let credentials_list = anoncreds::prover_fetch_next_credentials_for_proof_req(
            search_handle,
            attr1_referent,
            1,
        )
        ...
        let cred_info_value = credentials_first
            .as_object()
            .unwrap()
            .get("cred_info")
            .unwrap();
        ...
        let (_, revoc_reg_delta_json, timestamp) =
            pool.get_revoc_reg_delta(None, &rev_reg_id.0, from, to);

        // Prover creates RevocationState
        let prover_blob_storage_reader_handle =
            blob_storage::open_reader(TYPE, &anoncreds::tails_writer_config()).unwrap();

        let rev_state_json = anoncreds::create_revocation_state(
            prover_blob_storage_reader_handle,
            &revoc_reg_def_json,
        ...
        let requested_predicates = !proof_request_value
            .as_object()
            .unwrap()
...
impl Verifier {
    ...
    pub fn verify(&self, pool: &Pool, proof_json: &str) -> bool {
        ...
        let (_, revoc_reg_def_json) =
            pool.get_revoc_reg_def(Some(DID_MY1), &identifier.rev_reg_id.clone().unwrap().0);

        // Verifier gets RevocationRegistry from Ledger
        let (_, rev_reg_json, timestamp) = pool.get_revoc_reg_delta(
            Some(DID_MY1),
            &identifier.rev_reg_id.clone().unwrap().0,
            None,
            identifier.timestamp.unwrap(),
        );
...
fn anoncreds_revocation_interaction_test_one_prover(revocation_registry_config: &str) {
    ...
    let (cred_json, cred_rev_id, _revoc_reg_delta_json) = issuer.issue_credential(
        &pool,
        &cred_offer_json,
        &cred_req_json,
    ...
    // Basic check
    let credentials = anoncreds::prover_get_credentials(
        prover.wallet_handle,
...
#[cfg(not(feature = "only_high_cases"))]
fn multi_steps_create_revocation_credential(
    pool: &Pool,
    issuer: &Issuer,
    prover: &mut Prover,
    cred_values_json: &str,
    ...
    // Issuer issues credential
    let (cred_json, cred_rev_id, revoc_reg_delta_json) =
        issuer.issue_credential(&pool, &cred_offer_json, &cred_req_json, cred_values_json);
...
fn anoncreds_revocation_interaction_test_issuance_by_demand_three_credentials_post_entry_three_times_proving_first(
    ...
    // ISSUER post to Ledger Schema, CredentialDefinition, RevocationRegistry
    issuer.create_initial_ledger_state(
        &pool,
        r#"{"max_cred_num":5, "issuance_type":"ISSUANCE_ON_DEMAND"}"#,
    );
    ...
    let (_prover1_cred_rev_id, _prover1_revoc_reg_delta1_json) =
        multi_steps_create_revocation_credential(
            &pool,
            &issuer,
            &mut prover1,
            &anoncreds::gvt_credential_values_json(),
...
fn anoncreds_revocation_interaction_test_issuance_by_demand_three_credentials_post_common_entry_proving_all(
    ...
    let (_prover1_cred_rev_id, revoc_reg_delta1_json) = multi_steps_create_revocation_credential(
        &pool,
        &issuer,
        &mut prover1,
        &anoncreds::gvt_credential_values_json(),
    ...
    let (_prover2_cred_rev_id, revoc_reg_delta2_json) = multi_steps_create_revocation_credential(
        &pool,
        &issuer,
        &mut prover2,
        &anoncreds::gvt2_credential_values_json(),
    ...
    let (_prover3_cred_rev_id, revoc_reg_delta3_json) = multi_steps_create_revocation_credential(
        &pool,
        &issuer,
        &mut prover3,
        &anoncreds::gvt3_credential_values_json(),
    ...
    // Issuer merge Revocation Registry Deltas
    let _revoc_reg_delta_json = anoncreds::issuer_merge_revocation_registry_deltas(
        &revoc_reg_delta_json,
...
fn anoncreds_revocation_interaction_test_issuance_by_demand_fully_qualified_did() {
    ...
    let mut issuer = Issuer {
        ...
        // Issuer create DID
        issuer_wallet_config: wallet_config,
        issuer_did: did::create_store_and_publish_my_did_from_trustee_v1(
            wallet_handle,
            pool.pool_handle,
        )
        .unwrap()
    ...
    let (prover_did, prover_verkey) =
        did::create_my_did(prover_wallet_handle, &my_did_json).unwrap();
    ...
    // Issuer issues credential
    let (cred_json, _, _) = issuer.issue_credential(
        &pool,
        &cred_offer_json,
        &cred_req_json,
        &anoncreds::gvt_credential_values_json(),
