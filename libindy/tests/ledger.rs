...
use utils::{
    constants::*,
    domain::anoncreds::credential_definition::CredentialDefinitionV1,
    domain::{
        anoncreds::{
            revocation_registry::RevocationRegistryV1,
            revocation_registry_definition::RevocationRegistryDefinitionV1,
            revocation_registry_delta::RevocationRegistryDeltaV1, schema::SchemaV1,
        },
        crypto::did::DidValue,
        ledger::{constants, nym::NymData, request::DEFAULT_LIBIDY_DID},
    },
    types::*,
    Setup,
};
...
mod high_cases {
    ...
    mod requests {
        ...
        fn indy_sign_and_submit_request_works_for_invalid_pool_handle() {
            ...
            let res = ledger::sign_and_submit_request(
                INVALID_POOL_HANDLE,
                setup.wallet_handle,
                &setup.did,
                REQUEST,
        ...
        fn indy_submit_request_works() {
            ...
            let data: serde_json::Value =
                serde_json::from_str(reply["result"]["data"].as_str().unwrap()).unwrap();

            assert_eq!(data["dest"].as_str().unwrap(), "Th7MpTaRZVRYnPiabds81Y");

            assert_eq!(
                data["identifier"].as_str().unwrap(),
            ...
            assert_eq!(
                reply["result"]["identifier"].as_str().unwrap(),
                "Th7MpTaRZVRYnPiabds81Y"
            );

            assert_eq!(
                reply["result"]["dest"].as_str().unwrap(),
                "Th7MpTaRZVRYnPiabds81Y"
            );
        }
        ...
        fn indy_sign_and_submit_request_works() {
            ...
            let nym_request =
                ledger::build_nym_request(&setup.did, &did, None, None, None).unwrap();

            let nym_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &nym_request,
    ...
    mod submit_action {
        ...
        fn indy_submit_action_works_for_pool_restart() {
            ...
            let pool_request_request =
                ledger::build_pool_restart_request(&setup.did, "start", None).unwrap();

            let pool_request_request =
                ledger::sign_request(setup.wallet_handle, &setup.did, &pool_request_request)
                    .unwrap();
        ...
        fn indy_submit_action_works_for_list_nodes() {
            ...
            let get_validator_info_request =
                ledger::build_get_validator_info_request(&setup.did).unwrap();

            let get_validator_info_request =
                ledger::sign_request(setup.wallet_handle, &setup.did, &get_validator_info_request)
                    .unwrap();

            let nodes = r#"["Node1", "Node2"]"#;

            let response = ledger::submit_action(
                setup.pool_handle,
                &get_validator_info_request,
                Some(nodes),
                None,
            )
            .unwrap();
    ...
    mod multi_sign_request {
        ...
        fn indy_multi_sign_request_works() {
            ...
            let (did2, _) =
                did::create_and_store_my_did(setup.wallet_handle, Some(MY1_SEED)).unwrap();
            ...
            assert_eq!(
                signatures[DID_TRUSTEE],
                r#"65hzs4nsdQsTUqLCLy2qisbKLfwYKZSWoyh1C6CU59p5pfG3EHQXGAsjW4Qw4QdwkrvjSgQuyv8qyABcXRBznFKW"#
            );

            assert_eq!(
                signatures[DID_MY1],
                r#"49aXkbrtTE3e522AefE76J51WzUiakw3ZbxxWzf44cv7RS21n8mMr4vJzi4TymuqDupzCz7wEtuGz6rA94Y73kKR"#
            );
        }
        ...
        fn indy_multi_sign_request_works_for_fully_qualified() {
            ...
            let (did1, _) =
                did::create_and_store_my_did_v1(setup.wallet_handle, Some(TRUSTEE_SEED)).unwrap();

            let (did2, _) =
                did::create_and_store_my_did_v1(setup.wallet_handle, Some(MY1_SEED)).unwrap();
        ...
        fn indy_multi_sign_request_works_for_start_from_single_signature() {
            ...
            let message =
                ledger::sign_request(setup.wallet_handle, &did, REQUEST_FROM_TRUSTEE).unwrap();
            ...
            assert_eq!(
                signatures[DID_TRUSTEE],
                r#"3YnLxoUd4utFLzeXUkeGefAqAdHUD7rBprpSx2CJeH7gRYnyjkgJi7tCnFgUiMo62k6M2AyUDtJrkUSgHfcq3vua"#
            );

            assert_eq!(
                signatures[DID_MY1],
                r#"4EyvSFPoeQCJLziGVqjuMxrbuoWjAWUGPd6LdxeZuG9w3Bcbt7cSvhjrv8SX5e8mGf8jrf3K6xd9kEhXsQLqUg45"#
            );
        }
    ...
    mod nym_requests {
        ...
        fn indy_build_nym_requests_works_with_option_fields() {
            ...
            let request = ledger::build_nym_request(
                &IDENTIFIER,
                &DEST,
                Some(VERKEY_TRUSTEE),
                Some(alias),
        ...
        fn indy_build_nym_requests_works_for_empty_role() {
            ...
            let request =
                ledger::build_nym_request(&IDENTIFIER, &DEST, None, None, Some("")).unwrap();
        ...
        fn indy_build_nym_requests_works_for_fully_qualified() {
            ...
            let request =
                ledger::build_nym_request(&IDENTIFIER_V1, &DEST_V1, None, None, None).unwrap();
        ...
        fn indy_send_get_nym_request_works() {
            ...
            let get_nym_request =
                ledger::build_get_nym_request(Some(&setup.did), &setup.did).unwrap();
        ...
        fn indy_nym_requests_works() {
            ...
            let nym_request =
                ledger::build_nym_request(&setup.did, &my_did, Some(&my_verkey), None, None)
                    .unwrap();
            let nym_resp = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &nym_request,
            ...
            let get_nym_response =
                ledger::submit_request_with_retries(setup.pool_handle, &get_nym_request, &nym_resp)
                    .unwrap();
    ...
    mod attrib_requests {
        ...
        fn indy_build_attrib_requests_works_for_raw_value() {
            ...
            let request =
                ledger::build_attrib_request(&IDENTIFIER, &DEST, None, Some(ATTRIB_RAW_DATA), None)
                    .unwrap();
        ...
        fn indy_build_attrib_requests_works_for_hash_value() {
            ...
            let request = ledger::build_attrib_request(
                &IDENTIFIER,
                &DEST,
                Some(ATTRIB_HASH_DATA),
                None,
                None,
            )
            .unwrap();
        ...
        fn indy_build_attrib_requests_works_for_enc_value() {
            ...
            let request =
                ledger::build_attrib_request(&IDENTIFIER, &DEST, None, None, Some(ATTRIB_ENC_DATA))
                    .unwrap();
        ...
        fn indy_build_get_attrib_requests_works_for_raw_value() {
            ...
            let request =
                ledger::build_get_attrib_request(Some(IDENTIFIER), &DEST, Some(raw), None, None)
                    .unwrap();
        ...
        fn indy_build_get_attrib_requests_works_for_default_submitter_did() {
            let request =
                ledger::build_get_attrib_request(None, &DEST, Some(ATTRIB_RAW_DATA), None, None)
                    .unwrap();
        ...
        fn indy_attrib_requests_works_for_raw_value() {
            ...
            let attrib_request = ledger::build_attrib_request(
                &setup.did,
                &setup.did,
                None,
                Some(ATTRIB_RAW_DATA),
            ...
            let attrib_req_resp = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &attrib_request,
            ...
            let get_attrib_request = ledger::build_get_attrib_request(
                Some(&setup.did),
                &setup.did,
                Some("endpoint"),
                None,
            ...
            let get_attrib_response = ledger::submit_request_with_retries(
                setup.pool_handle,
                &get_attrib_request,
                &attrib_req_resp,
            )
            ...
            let get_attrib_response: Reply<GetAttribReplyResult> =
                serde_json::from_str(&get_attrib_response).unwrap();

            assert_eq!(
                get_attrib_response.result.data.unwrap().as_str(),
                ATTRIB_RAW_DATA
            );
        }
    }
    ...
    mod schema_requests {
        ...
        fn indy_build_get_schema_requests_works_for_correct_data_json() {
            ...
            let request =
                ledger::build_get_schema_request(Some(IDENTIFIER), &anoncreds::gvt_schema_id())
                    .unwrap();
        ...
        fn indy_build_get_schema_requests_works_for_fully_qualified() {
            ...
            let request = ledger::build_get_schema_request(
                Some(IDENTIFIER_V1),
                &anoncreds::gvt_schema_id_fully_qualified(),
            )
            .unwrap();
        ...
        fn indy_build_get_schema_requests_works_for_default_submitter_did() {
            let request =
                ledger::build_get_schema_request(None, &anoncreds::gvt_schema_id()).unwrap();
        ...
        fn indy_schema_requests_works() {
            ...
            let get_schema_request =
                ledger::build_get_schema_request(Some(DID_MY1), &schema_id).unwrap();

            let get_schema_response =
                ledger::submit_request(setup.pool_handle, &get_schema_request).unwrap();
    ...
    mod node_request {
        ...
        fn indy_submit_node_request_works_for_new_steward() {
            ...
            let (my_did, _) = did::create_store_and_publish_my_did_from_steward(
                setup.wallet_handle,
                setup.pool_handle,
            )
            .unwrap();
    ...
    mod cred_def_requests {
        ...
        fn indy_build_get_cred_def_request_works_for_fully_qualified() {
            ...
            let request = ledger::build_get_cred_def_request(
                Some(IDENTIFIER_V1),
                &anoncreds::gvt_cred_def_id_fully_qualified(),
            )
            .unwrap();
        ...
        fn indy_cred_def_requests_works() {
            ...
            let get_cred_def_request =
                ledger::build_get_cred_def_request(Some(DID_MY1), &cred_def_id).unwrap();

            let get_cred_def_response =
                ledger::submit_request(setup.pool_handle, &get_cred_def_request).unwrap();
    ...
    mod get_validator_info {
        ...
        fn indy_get_validator_info_request_works() {
            ...
            let get_validator_info_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &get_validator_info_request,
            ...
            let get_validator_info_response: HashMap<String, String> =
                serde_json::from_str(&get_validator_info_response).unwrap();
    ...
    mod get_txn_requests {
        ...
        fn indy_build_get_txn_request_for_ledger_type_as_number() {
            ...
            let request =
                ledger::build_get_txn_request(Some(IDENTIFIER), SEQ_NO, Some("10")).unwrap();
        ...
        fn indy_build_get_txn_request_for_ledger_type() {
            ...
            let request =
                ledger::build_get_txn_request(Some(IDENTIFIER), SEQ_NO, Some("POOL")).unwrap();
        ...
        fn indy_get_txn_request_works() {
            ...
            let schema_request =
                ledger::build_schema_request(&setup.did, &anoncreds::gvt_schema_json()).unwrap();

            let schema_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &schema_request,
            ...
            let get_txn_request =
                ledger::build_get_txn_request(Some(&setup.did), seq_no, None).unwrap();

            let get_txn_response =
                ledger::submit_request(setup.pool_handle, &get_txn_request).unwrap();

            let get_txn_response: Reply<GetTxnResult> =
                serde_json::from_str(&get_txn_response).unwrap();

            let get_txn_schema_data: SchemaData =
                serde_json::from_value(serde_json::Value::Object(
                    get_txn_response.result.data.unwrap()["txn"]["data"]["data"]
                        .as_object()
                        .unwrap()
                        .clone(),
                ))
            ...
            let expected_schema_data: SchemaData = serde_json::from_str(
                r#"{"name":"gvt","version":"1.0","attr_names":["name", "age", "sex", "height"]}"#,
            )
            .unwrap();
    ...
    mod pool_restart {
        ...
        fn indy_build_pool_restart_request_works_for_start_action() {
            ...
            let request =
                ledger::build_pool_restart_request(DID_TRUSTEE, "start", Some("0")).unwrap();
        ...
        fn indy_pool_restart_request_works_for_start_cancel_works() {
            ...
            //start
            let request =
                ledger::build_pool_restart_request(&setup.did, "start", Some(&DATETIME)).unwrap();
    ...
    mod pool_upgrade {
        ...
        lazy_static! {
            static ref SCHEDULE: String = {
                let next_year = time::now().tm_year + 1900 + 1;
                format!(
                    r#"{{"Gw6pDLhcBcoQesN72qfotTgFa7cbuqZpkX3Xo6pLhPhv":"{}-01-25T12:49:05.258870+00:00",
    ...
    mod revoc_reg_def_requests {
        ...
        fn indy_build_get_revoc_reg_def_request() {
            ...
            let request =
                ledger::build_get_revoc_reg_def_request(Some(DID), &anoncreds::gvt_rev_reg_id())
                    .unwrap();
        ...
        fn indy_build_get_revoc_reg_def_request_for_fully_qualified() {
            ...
            let request = ledger::build_get_revoc_reg_def_request(
                Some(DID_V1),
                &anoncreds::gvt_rev_reg_id_fully_qualified(),
            )
            .unwrap();
        ...
        fn indy_build_get_revoc_reg_def_request_for_default_submitter_did() {
            let request =
                ledger::build_get_revoc_reg_def_request(None, &anoncreds::gvt_rev_reg_id())
                    .unwrap();
        ...
        fn indy_revoc_reg_def_requests_works() {
            ...
            let get_rev_reg_def_request =
                ledger::build_get_revoc_reg_def_request(Some(DID_MY1), &rev_reg_id).unwrap();

            let get_rev_reg_def_response =
                ledger::submit_request(setup.pool_handle, &get_rev_reg_def_request).unwrap();

            let (_, revoc_reg_def_json) =
                ledger::parse_get_revoc_reg_def_response(&get_rev_reg_def_response).unwrap();

            let _revoc_reg_def: RevocationRegistryDefinitionV1 =
                serde_json::from_str(&revoc_reg_def_json).unwrap();
        }
    }
    ...
    mod revoc_reg_entry_request {
        ...
        fn indy_build_revoc_reg_entry_request() {
            ...
            let request = ledger::build_revoc_reg_entry_request(
                DID,
    ...
    mod get_revoc_reg_request {
        ...
        fn indy_build_get_revoc_reg_request() {
            ...
            let request =
                ledger::build_get_revoc_reg_request(Some(DID), &anoncreds::gvt_rev_reg_id(), 100)
                    .unwrap();
        ...
        fn indy_build_get_revoc_reg_request_for_fully_qualified() {
            ...
            let request = ledger::build_get_revoc_reg_request(
                Some(DID_V1),
                &anoncreds::gvt_rev_reg_id_fully_qualified(),
                100,
            )
        ...
        fn indy_build_get_revoc_reg_request_for_default_submitter_did() {
            let request =
                ledger::build_get_revoc_reg_request(None, &anoncreds::gvt_rev_reg_id(), 100)
                    .unwrap();
        ...
        fn indy_get_revoc_reg_request_works() {
            ...
            let get_rev_reg_req =
                ledger::build_get_revoc_reg_request(Some(DID_MY1), &rev_reg_id, timestamp).unwrap();

            let get_rev_reg_resp =
                ledger::submit_request(setup.pool_handle, &get_rev_reg_req).unwrap();

            let (_, revoc_reg_json, _) =
                ledger::parse_get_revoc_reg_response(&get_rev_reg_resp).unwrap();
    ...
    mod get_revoc_reg_delta_request {
        ...
        fn indy_build_get_revoc_reg_delta_request() {
            ...
            let request = ledger::build_get_revoc_reg_delta_request(
                Some(DID),
                &anoncreds::gvt_rev_reg_id(),
                None,
                100,
        ...
        fn indy_get_revoc_reg_delta_request_works() {
            ...
            let get_rev_reg_delta_req =
                ledger::build_get_revoc_reg_delta_request(Some(DID_MY1), &rev_reg_id, None, to)
                    .unwrap();

            let get_rev_reg_delta_resp =
                ledger::submit_request(setup.pool_handle, &get_rev_reg_delta_req).unwrap();

            let (_, revoc_reg_delta_json, _) =
                ledger::parse_get_revoc_reg_delta_response(&get_rev_reg_delta_resp).unwrap();

            let _revoc_reg_delta: RevocationRegistryDeltaV1 =
                serde_json::from_str(&revoc_reg_delta_json).unwrap();
        }
        ...
        fn indy_get_revoc_reg_delta_request_works_for_two_timestamps() {
            ...
            let get_rev_reg_delta_req = ledger::build_get_revoc_reg_delta_request(
                Some(DID_MY1),
                &rev_reg_id,
                Some(from),
                to,
    ...
    // FIXME: [async] re-check if needed
    ...
    mod auth_rule {
        ...
        fn indy_build_auth_rule_requests_works_for_adding_new_trustee() {
            ...
            let request = ledger::build_auth_rule_request(
                DID_TRUSTEE,
                constants::NYM,
                &ADD_AUTH_ACTION,
                FIELD,
        ...
        fn indy_build_auth_rule_request_works_for_invalid_constraint() {
            let res = ledger::build_auth_rule_request(
                DID_TRUSTEE,
                constants::NYM,
                &ADD_AUTH_ACTION,
                FIELD,
        ...
        fn indy_auth_rule_requests_works_for_adding_new_trustee() {
            ...
            let (_, default_constraint_json) = _get_constraint(
                setup.pool_handle,
                ADD_AUTH_ACTION,
                constants::NYM,
                FIELD,
            ...
            let (actual_constraint, _) = _get_constraint(
                setup.pool_handle,
                ADD_AUTH_ACTION,
                constants::NYM,
                FIELD,
            ...
            let expected_constraint: serde_json::Value =
                serde_json::from_str(ROLE_CONSTRAINT).unwrap();
        ...
        fn _change_constraint(
            ...
            let auth_rule_request = ledger::build_auth_rule_request(
                &trustee_did,
                txn_type,
                &action,
                field,
        ...
        fn _get_constraint(
            ...
            let get_auth_rule_request = ledger::build_get_auth_rule_request(
                None,
                Some(txn_type),
                Some(action),
                Some(field),
        ...
        fn indy_get_auth_rule_request_works_for_getting_all() {
            ...
            let get_auth_rule_request =
                ledger::build_get_auth_rule_request(None, None, None, None, None, None).unwrap();

            let response =
                ledger::submit_request(setup.pool_handle, &get_auth_rule_request).unwrap();
        ...
        fn indy_auth_rules_request_works() {
            ...
            let action1: (&str, &str, &str, Option<&str>, Option<&str>) =
                (ADD_AUTH_ACTION, constants::NYM, FIELD, None, Some(VALUE));

            let action2: (&str, &str, &str, Option<&str>, Option<&str>) = (
                EDIT_AUTH_ACTION,
                constants::NYM,
                FIELD,
                Some(VALUE),
            ...
            let (_, default_constraint_action_1) = _get_constraint(
                setup.pool_handle,
                action1.0,
                action1.1,
                action1.2,
            ...
            let (_, default_constraint_action_2) = _get_constraint(
                setup.pool_handle,
                action2.0,
                action2.1,
                action2.2,
            ...
            let auth_rule_request =
                ledger::build_auth_rules_request(&setup.did, &data.to_string()).unwrap();
    ...
    mod author_agreement {
        ...
        fn indy_build_txn_author_agreement_request() {
            ...
            let request = ledger::build_txn_author_agreement_request(
                DID_TRUSTEE,
                Some(TEXT),
                VERSION,
                None,
        ...
        fn indy_build_get_txn_author_agreement_request_for_digest() {
            ...
            let data = json!({ "digest": TAA_DIGEST }).to_string();

            let request =
                ledger::build_get_txn_author_agreement_request(None, Some(&data)).unwrap();
        ...
        fn indy_build_get_txn_author_agreement_request_for_timestamp() {
            ...
            let data = json!({ "timestamp": timestamp }).to_string();
    ...
    mod acceptance_mechanism {
        ...
        fn indy_build_acceptance_mechanisms_request() {
            ...
            let request = ledger::build_acceptance_mechanisms_request(
                DID_TRUSTEE,
                &aml.to_string(),
                VERSION,
                None,
        ...
        fn indy_build_get_acceptance_mechanisms_request() {
            ...
            let request =
                ledger::build_get_acceptance_mechanisms_request(None, None, None).unwrap();
        ...
        fn indy_build_get_acceptance_mechanisms_request_for_timestamp() {
            ...
            let request =
                ledger::build_get_acceptance_mechanisms_request(None, Some(timestamp), None)
                    .unwrap();
        ...
        fn indy_build_get_acceptance_mechanisms_request_for_version() {
            ...
            let request =
                ledger::build_get_acceptance_mechanisms_request(None, None, Some(VERSION)).unwrap();
    ...
    mod author_agreement_acceptance {
        ...
        fn indy_append_txn_author_agreement_acceptance_to_request_works_for_text_version() {
            ...
            let request = ledger::append_txn_author_agreement_acceptance_to_request(
                REQUEST,
                Some(TEXT),
                Some(VERSION),
                None,
        ...
        fn indy_append_txn_author_agreement_acceptance_to_request_works_for_text_version_not_correspond_to_hash(
            ...
            let res = ledger::append_txn_author_agreement_acceptance_to_request(
                REQUEST,
                Some("other text"),
                Some("0.0.1"),
                Some(HASH),
        ...
        fn _gen_aml_data() -> (serde_json::Value, String, String, String) {
            ...
            let aml = json!({ aml_label.clone(): _rand_string() });
        ...
        fn _send_taa(
            ...
            let response =
                ledger::sign_and_submit_request(pool_handle, wallet_handle, &trustee_did, &request)
                    .unwrap();
        ...
        fn _disable_taa(pool_handle: PoolHandle, wallet_handle: WalletHandle, trustee_did: &str) {
            let request =
                ledger::build_disable_all_txn_author_agreements_request(&trustee_did).unwrap();
            let response =
                ledger::sign_and_submit_request(pool_handle, wallet_handle, &trustee_did, &request)
                    .unwrap();
            pool::check_response_type(&response, ResponseType::REPLY);
        ...
        fn _check_taa(
            ...
            let get_txn_author_agreement_response = ledger::submit_request_with_retries(
                pool_handle,
        ...
        fn indy_txn_author_agreement_requests_works() {
            ...
            let txn_author_agreement_request = ledger::build_txn_author_agreement_request(
                &setup.did,
                Some(&taa_text),
                &taa_version,
                Some(ratification_ts),
            ...
            let txn_author_agreement_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &txn_author_agreement_request,
            )
            .unwrap();
        ...
        fn indy_acceptance_mechanism_requests_works() {
            ...
            let acceptance_mechanisms_request = ledger::build_acceptance_mechanisms_request(
                &setup.did,
                &aml.to_string(),
                &aml_version,
                Some(&aml_context),
            ...
            let acceptance_mechanisms_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &acceptance_mechanisms_request,
            )
            .unwrap();
...

            {
                ...
                let response = ledger::submit_request_with_retries(
                    setup.pool_handle,
                    &request,
                    &acceptance_mechanisms_response,
                )
                .unwrap();
                ...
                let expected_data =
                    json!({"aml": aml, "version": aml_version, "amlContext": aml_context});
        ...
        fn indy_author_agreement_works() {
            ...
            let nym_req =
                ledger::build_nym_request(&setup.did, &did_, Some(&verkey_), None, None).unwrap();
            ...
            let nym_req = ledger::append_txn_author_agreement_acceptance_to_request(
                &nym_req,
                Some(&taa_text),
                Some(&taa_version),
                None,
            ...
            let get_nym_resp =
                ledger::submit_request_with_retries(setup.pool_handle, &get_nym_req, &nym_resp)
                    .unwrap();
        ...
        fn indy_reset_author_agreement_works() {
            ...
            {
                let nym_req =
                    ledger::build_nym_request(&setup.did, &did_, Some(&verkey_), None, None)
                        .unwrap();
        ...
        fn indy_author_agreement_works_for_using_invalid_aml() {
            ...
            let (taa_text, taa_version, _taa_digest, _) =
                _set_taa(setup.pool_handle, setup.wallet_handle, &setup.did);
        ...
        fn indy_txn_author_agreement_requests_works_for_set_retirement_ts() {
            ...
            let txn_author_agreement_response = _send_taa(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &taa_text,
        ...
        fn indy_author_agreement_works_for_using_not_latest_taa() {
            ...
            let (taa_text_2, taa_version_2, _, _) =
                _set_taa(setup.pool_handle, setup.wallet_handle, &setup.did);
    ...
    mod append_request_endorser {
        ...
        fn _setup_new_identity(wallet_handle: WalletHandle, pool_handle: PoolHandle) -> String {
            ...
            let nym =
                ledger::build_nym_request(DID_TRUSTEE, &my_did, Some(&my_vk), None, None).unwrap();

            let response =
                ledger::sign_and_submit_request(pool_handle, wallet_handle, DID_TRUSTEE, &nym)
                    .unwrap();
        ...
        fn indy_send_request_by_endorser_works() {
            ...
            let request =
                ledger::multi_sign_request(setup.wallet_handle, &my_did, &request).unwrap();

            let request =
                ledger::multi_sign_request(setup.wallet_handle, &setup.did, &request).unwrap();
        ...
        fn indy_send_request_by_endorser_for_both_author_and_endorser_must_sign() {
            ...
            let request_2 =
                ledger::multi_sign_request(setup.wallet_handle, &setup.did, &request_2).unwrap();
...
mod medium_cases {
    ...
    mod submit_action {
        ...
        fn indy_submit_action_works_for_validator_info() {
            ...
            ledger::submit_action(setup.pool_handle, &get_validator_info_request, None, None)
                .unwrap();
        }
        ...
        fn indy_submit_action_works_for_pool_restart_for_invalid_pool_handle() {
            ...
            let res =
                ledger::submit_action(INVALID_POOL_HANDLE, &get_validator_info_request, None, None);
    ...
    mod multi_sign_request {
        ...
        fn indy_multi_sign_request_works_for_invalid_message_format() {
            ...
            let res =
                ledger::multi_sign_request(setup.wallet_handle, &setup.did, "1495034346617224651");
    ...
    mod nym_requests {
        ...
        fn indy_send_nym_request_works_for_only_required_fields() {
            ...
            let nym_request =
                ledger::build_nym_request(&setup.did, &my_did, None, None, None).unwrap();
        ...
        fn indy_send_nym_request_works_with_option_fields() {
            ...
            let nym_request = ledger::build_nym_request(
                &setup.did,
                &my_did,
                Some(&my_verkey),
                Some("some_alias"),
        ...
        fn indy_send_nym_request_works_for_different_roles() {
            ...
            .iter()
            {
                ...
                let nym_request =
                    ledger::build_nym_request(&setup.did, &my_did, None, None, Some(role)).unwrap();
        ...
        fn indy_nym_request_works_for_wrong_signer_role() {
            ...
            let nym_request =
                ledger::build_nym_request(&my_did, &my_did2, None, None, None).unwrap();
        ...
        fn indy_nym_requests_works_for_reset_role() {
            ...
            let mut nym_request = ledger::build_nym_request(
                &setup.did,
                &my_did,
                Some(&my_verkey),
                None,
            ...
            let nym_req_resp = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &nym_request,
            ...
            let mut get_nym_request =
                ledger::build_get_nym_request(Some(&my_did), &my_did).unwrap();

            let get_nym_response_with_role = ledger::submit_request_with_retries(
                setup.pool_handle,
                &get_nym_request,
                &nym_req_resp,
            )
            ...
            let get_nym_response_data_with_role =
                ledger::parse_get_nym_response(&get_nym_response_with_role).unwrap();

            let get_nym_response_data_with_role: NymData =
                serde_json::from_str(&get_nym_response_data_with_role).unwrap();

            nym_request =
                ledger::build_nym_request(&my_did, &my_did, Some(&my_verkey), None, Some(""))
                    .unwrap();
            ...
            let get_nym_response_without_role = ledger::submit_request_with_retries(
                setup.pool_handle,
                &get_nym_request,
                &nym_req_resp,
            )
            ...
            let get_nym_response_data_without_role =
                ledger::parse_get_nym_response(&get_nym_response_without_role).unwrap();

            let get_nym_response_data_without_role: NymData =
                serde_json::from_str(&get_nym_response_data_without_role).unwrap();

            assert!(get_nym_response_data_without_role.role.is_none());

            assert_ne!(
                get_nym_response_data_without_role.role,
                get_nym_response_data_with_role.role
            );
        }
    }
    ...
    mod attrib_requests {
        ...
        fn indy_attrib_requests_works_for_encrypted_value() {
            ...
            let encryted_attr =
                hex::encode(secretbox::seal(&ATTRIB_RAW_DATA.as_bytes(), &nonce, &key));
        ...
        fn indy_attrib_request_works_for_unknown_did() {
            ...
            let attrib_request =
                ledger::build_attrib_request(&did, &did, None, Some(ATTRIB_RAW_DATA), None)
                    .unwrap();
        ...
        fn indy_get_attrib_requests_works_for_default_submitter_did() {
            ...
            let get_attrib_request =
                ledger::build_get_attrib_request(None, &setup.did, Some("endpoint"), None, None)
                    .unwrap();
        ...
        fn indy_get_attrib_request_works_for_unknown_did() {
            ...
            let get_attrib_request =
                ledger::build_get_attrib_request(Some(&did), &did, Some("endpoint"), None, None)
                    .unwrap();

            let get_attrib_response =
                ledger::submit_request(setup.pool_handle, &get_attrib_request).unwrap();
        ...
        fn indy_build_attrib_request_works_for_invalid_submitter_did() {
            let res = ledger::build_attrib_request(
                INVALID_IDENTIFIER,
                IDENTIFIER,
                None,
                Some(ATTRIB_RAW_DATA),
        ...
        fn indy_build_get_attrib_request_works_for_invalid_submitter_identifier() {
            let res = ledger::build_get_attrib_request(
    ...
    mod schemas_requests {
        ...
        fn indy_build_schema_request_works_for_attrs_count_more_than_acceptable() {
            ...
            schema.attr_names = (0..MAX_ATTRIBUTES_COUNT + 1)
                .map(|i| i.to_string())
    ...
    mod node_requests {
        ...
        fn indy_send_node_request_works_without_signature() {
            ...
            let node_request =
                ledger::build_node_request(&setup.did, &setup.did, NODE_DATA).unwrap();
    ...
    mod cred_def_requests {
        ...
        fn indy_cred_def_request_works_without_signature() {
            ...
            let cred_def_request =
                ledger::build_cred_def_txn(&setup.did, &anoncreds::credential_def_json()).unwrap();
        ...
        fn indy_get_cred_def_requests_works_for_default_submitter_did() {
            ...
            let get_cred_def_request =
                ledger::build_get_cred_def_request(None, &cred_def_id).unwrap();
        ...
        fn indy_build_cred_def_request_works_for_invalid_submitter_did() {
            let res =
                ledger::build_cred_def_txn(INVALID_IDENTIFIER, &anoncreds::credential_def_json());
        ...
        fn indy_build_get_cred_def_request_works_for_invalid_submitter_did() {
            let res = ledger::build_get_cred_def_request(
                Some(INVALID_IDENTIFIER),
        ...
        fn indy_cred_def_requests_works_for_hash_field() {
            ...
            let get_schema_request =
                ledger::build_get_schema_request(Some(&setup.did), &schema_id).unwrap();

            let get_schema_response = ledger::submit_request_with_retries(
                setup.pool_handle,
                &get_schema_request,
                &schema_response,
            )
            ...
            let cred_def_response = ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
                &setup.did,
                &cred_def_request,
            ...
            let get_cred_def_response = ledger::submit_request_with_retries(
                setup.pool_handle,
    ...
    mod get_txn_requests {
        ...
        fn indy_get_txn_request_works_for_invalid_seq_no() {
            ...
            let get_txn_request =
                ledger::build_get_txn_request(Some(DID), seq_no + 1, None).unwrap();
        ...
        fn indy_get_txn_request_works_for_attrib_request_raw_value() {
            ...
            let data: serde_json::Value = serde_json::from_value(serde_json::Value::Object(
                get_txn_response.result.data.unwrap()["txn"]["data"]
                    .as_object()
                    .unwrap()
                    .clone(),
            ))
    ...
    mod revoc_reg_def_requests {
        ...
        fn indy_revoc_get_reg_def_requests_works_for_default_submitter_did() {
            ...
            let get_rev_reg_def_request =
                ledger::build_get_revoc_reg_def_request(None, &rev_reg_id).unwrap();
    ...
    mod get_revoc_reg_request {
        ...
        fn indy_get_revoc_reg_request_works_for_default_submitter_did() {
            ...
            let get_rev_reg_req =
                ledger::build_get_revoc_reg_request(None, &rev_reg_id, timestamp).unwrap();
    ...
    mod get_revoc_reg_delta_request {
        ...
        fn indy_get_revoc_reg_delta_request_works_for_default_submitter_did() {
            ...
            let get_rev_reg_delta_req =
                ledger::build_get_revoc_reg_delta_request(None, &rev_reg_id, None, to).unwrap();
