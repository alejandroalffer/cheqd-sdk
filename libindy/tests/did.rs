...
pub const SIGNATURE: &'static [u8; 64] = &[
    20, 191, 100, 213, 101, 12, 197, 198, 203, 49, 89, 220, 205, 192, 224, 221, 97, 77, 220, 190,
    90, 60, 142, 23, 16, 240, 189, 129, 45, 148, 245, 8, 102, 95, 95, 249, 100, 89, 41, 227, 213,
    25, 100, 1, 232, 188, 245, 235, 186, 21, 52, 176, 236, 11, 99, 70, 155, 159, 89, 215, 197, 239,
    138, 5,
];
...
mod high_cases {
    ...
    mod key_for_did {
        ...
        fn indy_key_for_did_works_for_get_key_from_ledger() {
            ...
            let received_verkey =
                did::key_for_did(setup.pool_handle, setup.wallet_handle, DID_TRUSTEE).unwrap();
    ...
    mod get_endpoint_for_did {
        ...
        fn indy_get_endpoint_for_did_works_for_fully_qualified_did() {
            ...
            did::set_endpoint_for_did(setup.wallet_handle, DID_V1, ENDPOINT, VERKEY).unwrap();
            let (endpoint, key) =
                did::get_endpoint_for_did(setup.wallet_handle, -1, DID_V1).unwrap();
            assert_eq!(ENDPOINT, endpoint);
            assert_eq!(VERKEY, key.unwrap());
        }
        ...
        fn indy_get_endpoint_for_did_works_from_ledger() {
            ...
            let attrib_data =
                json!({"endpoint": {"ha": ENDPOINT, "verkey": VERKEY_TRUSTEE}}).to_string();
            ...
            let (endpoint, key) =
                did::get_endpoint_for_did(setup.wallet_handle, setup.pool_handle, &setup.did)
                    .unwrap();
            assert_eq!(ENDPOINT, endpoint);
    ...
    mod create_my_did {
        ...
        fn indy_create_my_did_works_for_several_dids_but_different_methods() {
            ...
            let (my_did_1, my_verkey_1) =
                did::create_and_store_my_did(setup.wallet_handle, Some(MY1_SEED)).unwrap();

            let (my_did_2, my_verkey_2) =
                did::create_and_store_my_did_v1(setup.wallet_handle, Some(MY1_SEED)).unwrap();
            ...
            assert_eq!(
                my_verkey_1,
                did::key_for_local_did(setup.wallet_handle, &my_did_1).unwrap()
            );
            assert_eq!(
                my_verkey_2,
                did::key_for_local_did(setup.wallet_handle, &my_did_2).unwrap()
            );
            assert_eq!(
                my_verkey_3,
                did::key_for_local_did(setup.wallet_handle, &my_did_3).unwrap()
            );
        }
        ...
        fn indy_create_my_did_works_with_hex_seed() {
            ...
            let (my_did, my_verkey) = did::create_and_store_my_did(
                setup.wallet_handle,
                Some("94a823a6387cdd30d8f7687d95710ebab84c6e277b724790a5b221440beb7df6"),
            )
            .unwrap();
    ...
    mod replace_keys_start {
        ...
        fn indy_replace_keys_start_works_for_seed() {
            ...
            let new_verkey = did::replace_keys_start(
                setup.wallet_handle,
                &setup.did,
                r#"{"seed":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}"#,
            )
            .unwrap();
    ...
    mod replace_keys {
        ...
        fn indy_replace_keys_demo() {
            ...
            let nym_request =
                ledger::build_nym_request(&setup.did, &setup.did, Some(&new_verkey), None, None)
                    .unwrap();
            ledger::sign_and_submit_request(
                setup.pool_handle,
                setup.wallet_handle,
    ...
    mod abbreviate_verkey {
        ...
        fn indy_abbreviate_verkey_works_for_not_abbr_key() {
            ...
            let (did, verkey) = did::create_my_did(
                setup.wallet_handle,
                &format!(r#"{{"did":{:?}}}"#, DID_TRUSTEE),
            )
            .unwrap();
    ...
    mod qualify_did {
        ...
        fn qualify_did_for_appending_prefix() {
            ...
            let full_qualified_did =
                did::qualify_did(setup.wallet_handle, &setup.did, DEFAULT_METHOD_NAME).unwrap();
            assert_eq!(
                full_qualified_did,
                format!("{}{}", DEFAULT_PREFIX, setup.did)
            );
        }
        ...
        fn qualify_did_for_updating_prefix() {
            ...
            let new_full_qualified_did =
                did::qualify_did(setup.wallet_handle, &full_qualified_did, CUSTOM_METHOD).unwrap();
            assert_eq!(
                new_full_qualified_did,
                format!("did:{}:{}", CUSTOM_METHOD, setup.did)
            );
        }
        ...
        fn qualify_did_for_keeping_related_entities() {
            ...
            did::store_their_did(setup.wallet_handle, &identity_json).unwrap();
            utils::pairwise::create_pairwise(setup.wallet_handle, DID_TRUSTEE, &setup.did, None)
                .unwrap();
...

            {
                ...
                let verkey =
                    did::key_for_local_did(setup.wallet_handle, &full_qualified_did).unwrap();
...

            {
                ...
                let (endpoint, verkey) = did::get_endpoint_for_did(
                    setup.wallet_handle,
                    INVALID_POOL_HANDLE,
                    &full_qualified_did,
                )
...

            {
                ...
                let meta = did::get_my_did_with_metadata(setup.wallet_handle, &full_qualified_did)
                    .unwrap();
                let meta: serde_json::Value = serde_json::from_str(&meta).unwrap();
                assert_eq!(
                    temp_verkey,
                    meta["tempVerkey"].as_str().unwrap().to_string()
                );
            }

            {
                ...
                assert_eq!(
                    full_qualified_did,
                    pairwise["my_did"].as_str().unwrap().to_string()
                );

                // check pairwise 2
                let pairwise =
                    utils::pairwise::get_pairwise(setup.wallet_handle, DID_TRUSTEE).unwrap();
                let pairwise: serde_json::Value = serde_json::from_str(&pairwise).unwrap();
                assert_eq!(
                    full_qualified_did,
                    pairwise["my_did"].as_str().unwrap().to_string()
...
mod medium_cases {
    ...
    mod create_my_did {
        ...
        fn indy_create_my_did_works_as_cid() {
            ...
            let (my_did, my_verkey) = did::create_my_did(
                setup.wallet_handle,
                r#"{"seed":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","cid":true}"#,
            )
            .unwrap();
        ...
        fn indy_create_my_did_works_with_passed_did() {
            ...
            let (my_did, my_verkey) = did::create_my_did(
                setup.wallet_handle,
                &format!(
                    r#"{{"did":"{}","seed":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}}"#,
                    DID
                ),
            )
            .unwrap();
    ...
    mod store_their_did {
        ...
        fn indy_store_their_did_works_for_verkey_with_crypto_type() {
            ...
            let identity_json =
                json!({"did": DID, "verkey": VERKEY.to_owned() + ":ed25519"}).to_string();
            did::store_their_did(setup.wallet_handle, &identity_json).unwrap();
        }
        ...
        fn indy_store_their_did_works_for_abbreviated_verkey() {
            ...
            let identity_json =
                r#"{"did":"8wZcEriaNLNKtteJvx7f8i", "verkey":"~NcYxiDXkpYi6ov5FcYDi1e"}"#;
            did::store_their_did(setup.wallet_handle, identity_json).unwrap();
        }
        ...
        fn indy_store_their_did_works_for_verkey_with_invalid_crypto_type() {
            ...
            let identity_json =
                json!({"did": DID, "verkey": VERKEY.to_owned() + ":crypto_type"}).to_string();
            let res = did::store_their_did(setup.wallet_handle, &identity_json);
    ...
    mod replace_keys {
        ...
        fn indy_replace_keys_without_nym_transaction() {
            ...
            let (my_did, _) = did::create_store_and_publish_my_did_from_trustee(
                setup.wallet_handle,
                setup.pool_handle,
            )
            .unwrap();
    ...
    mod list_my_dids_with_meta {
        ...
        fn indy_list_dids() {
            ...
            assert_eq!(
                setup.verkey,
                info_list[0]["verkey"].as_str().unwrap().to_string()
            );
        }
        ...
        fn indy_list_dids_after_creating_dids() {
            ...
            for info in info_list.as_array().unwrap() {
                assert_eq!(
                    info["metadata"].as_str().unwrap().to_string(),
                    METADATA.to_string()
                );
                assert_eq!(
                    &info["verkey"].as_str().unwrap().to_string(),
                    did2verkey
                        .get(&(info["did"]).as_str().unwrap().to_string())
                        .unwrap()
                );
            }
        }
        ...
        fn indy_list_dids_after_replace_keys_start() {
            ...
            for info in info_list.as_array().unwrap() {
                ...
                assert_eq!(
                    &info["tempVerkey"].as_str().unwrap().to_string(),
