...
fn _add(
    ...
    ErrorCode::from(unsafe {
        cheqd_pool::indy_cheqd_pool_add(
            command_handle,
            alias.as_ptr(),
            rpc_address.as_ptr(),
            chain_id.as_ptr(),
...
fn _get_config(
    ...
    ErrorCode::from(unsafe {
        cheqd_pool::indy_cheqd_pool_get_config(command_handle, alias.as_ptr(), cb)
    })
}
...
fn _broadcast_tx_commit(
    ...
    ErrorCode::from(unsafe {
        cheqd_pool::indy_cheqd_pool_broadcast_tx_commit(
            command_handle,
            pool_alias.as_ptr(),
...
fn _abci_query(
    ...
    ErrorCode::from(unsafe {
        cheqd_pool::indy_cheqd_pool_abci_query(
            command_handle,
            pool_alias.as_ptr(),
            req_json.as_ptr(),
            cb,
...
fn _abci_info(
    ...
    ErrorCode::from(unsafe {
        cheqd_pool::indy_cheqd_pool_abci_info(
            command_handle,
            pool_alias.as_ptr(),
            cb,
        )
