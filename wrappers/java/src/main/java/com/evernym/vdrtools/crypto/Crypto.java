...
public class Crypto extends IndyJava.API {
	...
	private static Callback authDecryptCb = new Callback() {
		...
		public void callback(int xcommand_handle, int err, String their_vk, Pointer decrypted_msg_raw, int decrypted_msg_len) {
			...
			CompletableFuture<CryptoResults.AuthDecryptResult> future = (CompletableFuture<CryptoResults.AuthDecryptResult>) removeFuture(xcommand_handle);
			if (! checkResult(future, err)) return;

			byte[] decryptedMsg = new byte[decrypted_msg_len];
			decrypted_msg_raw.read(0, decryptedMsg, 0, decrypted_msg_len);

			CryptoResults.AuthDecryptResult result = new CryptoResults.AuthDecryptResult(their_vk, decryptedMsg);
	 ...
	 * @throws IndyException Thrown if an error occurs when calling the underlying SDK.
	 */
	public static CompletableFuture<CryptoResults.AuthDecryptResult> authDecrypt(
			Wallet wallet,
			String recipientVk,
		...
		CompletableFuture<CryptoResults.AuthDecryptResult> future = new CompletableFuture<CryptoResults.AuthDecryptResult>();
		int commandHandle = addFuture(future);
