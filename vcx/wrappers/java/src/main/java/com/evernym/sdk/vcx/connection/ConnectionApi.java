...
//            }
    ...
    public static CompletableFuture<Integer> vcxCreateConnectionWithInvite(String sourceId, String inviteDetails) throws VcxException {
        ParamGuard.notNullOrWhiteSpace(sourceId, "sourceId");
        ParamGuard.notNullOrWhiteSpace(inviteDetails, "inviteDetails");
        logger.debug("vcxCreateConnectionWithInvite() called with: sourceId = [" + sourceId + "], inviteDetails = [****]");
        CompletableFuture<Integer> future = new CompletableFuture<>();
        int commandHandle = addFuture(future);
